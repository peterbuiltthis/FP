...
(define value-of
  (lambda (expr env k)
    (union-case expr exp
                ...
                [(if test conseq alt)(value-of test env (lambda (v)
                                                          (if v
                                                              (value-of conseq env k) 
                                                              (value-of alt env k))))]
                [(mult rand1 rand2) (value-of rand1 env (lambda (v)
                                                          (value-of rand2 env (lambda (v^)
                                                                                (k (* v v^))))))]
                [(sub1 rand) (value-of rand env (lambda (v)
                                                  (k (- v 1))))]
                [(zero rand) (value-of rand env (lambda (v)
                                                  (k (zero? v))))]
                [(capture body)
                    (value-of body (envr_extend k env) k)]
                [(return vexp kexp)
                 (value-of kexp env (lambda (k)
                                      (value-of vexp env k)))]
                [(let vexp body)
                 (value-of vexp env (lambda (v)
                                      (value-of body (envr_extend v env) k)))]
                [(lambda body) (k (clos_closure body env))]
                [(app rator rand) (value-of rator env (lambda (v)
                                                        (value-of rand env (lambda (v^)
                                                                             (apply-closure v v^ k)))))])))
